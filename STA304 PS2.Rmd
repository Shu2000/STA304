---
title: "Linear Regression analysis of marriage age and age of giving birth"
author: "Shu Liu, Xinrui Wang"
date: "Oct. 19th"
output:
  pdf_document: 
...
To use the data, a procedure of cleaning data is processed. To achieve the goal, two variables: age_at_first_marriage and age_at_first_birth will be focused.

## Model

Since the focused two variables are all numerical variables, a scatterplot will be plotted to firstly discover their relationship. To make an estimation of the overall population, a linear regression will be made through least square method by rcode, which can help showing that whether they relate to each other linearly or not. This will result in a model, following the formula: 
...
Residuals and p-value testing will also be analyzed in the following sections to check if the model can fit the data. 
...
data.frame(anova(lmbr))

data.frame(confint(lmbr, level=0.95))
...
The null hypothesis in the analysis is that the age of first marriage does not affect the age of getting the first child. The alternative hypothesis is that the age of first marriage affects the age of the first child. The two variables should have a positive relationship. The result showed in the previous section gives us a positive value of the slope coefficient, which is 0.73254. This slope coefficient shows a ten years increase in age of first marriage is associated with a 7.33 years increase in the age of getting the first child, on average.
The t-statistics of the slope coefficient is 52.06, far more extensive than the 95% rejection critical value of 1.96. Thus, we have sufficient evidence to reject the null hypothesis and infer that the age of first marriage affects the age of getting the first child. 
However, when we look at the model's fitness to the data, the R^2 value is only 0.40, which means that variations of ages of first marriage can explain only 40% of the first child's variations in ages. The low R^2 value with a high statistical significance shows that there might be some possible omitted variables, which we will discuss in sections below.
